...
class Routes {
  ...
  async deletePost(session: WebSessionDoc, _id: ObjectId) {
	...
	const user = WebSession.getUser(session); //this is an ObjectID representing the user
	// _id is an objectID representing the post to be deleted
	// --> need to get the ID of the author of the post to be deleted
	const author = await Post.getAuthor(_id);
	if (user != author) {
